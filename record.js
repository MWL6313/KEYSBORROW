...
let sortAsc = false;          // 借用時間排序方向
let sortInspectionAsc = true; // 巡檢完成時間排序方向

// ===== Timeout thresholds (統一門檻) =====
const TIMEOUT_MS  = 60 * 60 * 1000;   // > 1 小時
const TIMEOUT0_MS = 30 * 60 * 1000;   // 0.5~1 小時
...
function getTimeoutFlags(borrowTimeStr) {
  ...
  return {
    ...
    timeout0: elapsed > TIMEOUT0_MS && elapsed <= TIMEOUT_MS,
    elapsedMs: elapsed
  };
}
...
// 顏色規則集中：已處置=綠；>1h 未處置=紅；0.5~1h 未處置=黃；巡檢正常=無底色
function applyRowColor(tr, record) {
  if (record.type === '手機') { tr.style.backgroundColor = ""; return; }
  ...
  if (hasAction) {
    ...
    tr.style.backgroundColor = "#ffdddd"; // 紅：>1 小時未處置
  } else if (timeout0) {
    tr.style.backgroundColor = "#fef9dc"; // 黃：0.5~1 小時未處置
  }
}

// 只要未巡檢正常且超過 0.5 小時，就顯示編輯鈕（不受 hasAction 影響）
function shouldShowEditBtn(record) {
  if (record.type === '手機') return false;
  if (!(currentRole === 'admin' || currentRole === 'manager')) return false;
  if (record.查核是否正常 === "巡檢正常") return false;
  const { timeout, timeout0 } = getTimeoutFlags(record.借用時間);
  return (timeout || timeout0);
}

// =========================== 主要流程放在 DOMContentLoaded ===========================
document.addEventListener("DOMContentLoaded", () => {
  ...
  // 先把可用的監聽都綁上（有元素才綁）
  if ($("inspectionFilter")) $("inspectionFilter").addEventListener("change", filterAndRender);
  if ($("searchUser")) $("searchUser").addEventListener("input", filterAndRender);
  if ($("searchCar")) $("searchCar").addEventListener("input", filterAndRender);
  ...
  if ($("sortInspectionBtn")) {
    $("sortInspectionBtn").onclick = () => {
      allRecords.sort((a, b) => {
  ...
  if ($("refreshBtn")) $("refreshBtn").addEventListener("click", reloadWithTimestamp);
  ...
  fetch("https://key-loan-api-299116105630.asia-east1.run.app/validateToken", {
    ...
    .then(async data => {
      if (data.success) {
        ...
        if ($("currentUserName")) {
          $("currentUserName").innerText = currentUser.name || currentUser.id || "";
        }
        if ($("inspectionFilter")) $("inspectionFilter").value = "incomplete"; // 預設只看未巡檢正常
  ...
  async function loadRecords() {
    ...
    try {
      ...
      if (!Array.isArray(data)) {
        if (statusMsg) statusMsg.innerText = "資料載入失敗，請稍後再試。";
        return;
      }
      ...
      if (!data2.success) {
        if (statusMsg) statusMsg.innerText = "無法取得使用者資訊。";
        return;
      }

      currentRole = data2.role || "";
      if ($("currentUserName")) {
        $("currentUserName").innerText = `${data2.user?.name || data2.user?.id || "(未知)"}`;
      }
      ...
      if (statusMsg) statusMsg.innerText = "無法連線伺服器。";
    }
  }

  // --------------------------- 篩選 + 繪製 ---------------------------
  function filterAndRender() {
    const searchUser = ($("searchUser")?.value || "").toLowerCase();
    const searchCar  = ($("searchCar")?.value || "").toLowerCase();
    const typeFilter = $("typeFilter")?.value || "all";
    const inspectionFilter = $("inspectionFilter")?.value || "all";

    const recordBody  = document.querySelector("#recordTable tbody");
    const historyBody = document.querySelector("#historyTable tbody");
    if (recordBody) recordBody.innerHTML = "";
    ...
    filteredRecords.forEach(record => {
      ...
      if (targetBody) renderRow(record, targetBody);
    });
  }
  ...
  function renderRow(record, tbody) {
    ...
    // 📝 編輯：未巡檢正常且 (timeout || timeout0) → 不管已處置與否都顯示
    if (shouldShowEditBtn(record)) {
  ...
  async function handleReturn(record) {
    ...
    try {
      ...
      const payload = record.type === '手機'
        ? { 借用人: record.借用人, 物品: record.物品, 借用時間: record.借用時間, 歸還原因: reason.trim() }
        : { 借用人: record.借用人, 車號: record.車號, 借用時間: record.借用時間, 歸還原因: reason.trim() };
  ...
  async function handleEditAbnormal(record) {
    ...
    //   title: "請輸入異常處置對策",
    //   input: "text",
    //   inputPlaceholder: "只須說明處置或補救措施(系統將自動編號並帶入登入帳號、時間)",
    //   showCancelButton: true,
    //   confirmButtonText: "確定",
    //   cancelButtonText: "取消"
    // });
    ...
    // 視覺：暫黃
    const tableBody = document.querySelector("#recordTable tbody");
    const targetRow = [...(tableBody?.querySelectorAll("tr") || [])].find(tr =>
      tr.children[0]?.innerText.trim() === record.借用人 &&
      tr.children[1]?.innerText.replace(/^📱|🚗/, "").trim() === (record.車號 || record.物品 || "-") &&
      tr.dataset.borrowTime === record.借用時間
    );
    if (targetRow) {
    ...
    try {
      ...
      if (result.success) {
        ...
        if (idx !== -1) {
          ...
          updateTableRow(allRecords[idx]); // 會轉為黃色，且編輯鈕仍保留（超過 0.5h）
        }
      } else {
  ...
  window.showToast = showToast; // 讓其它檔案能呼叫（不使用 export）
  ...
  function showChange(message) {
    ...
    if (latestChanges) latestChanges.style.display = "block";
